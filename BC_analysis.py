...
def gen_condition_time_list():
    ...
    for i in range(conditions["repeat"]):
        if i != 0:
            times_list.append(times_list[-1] + conditions["S"])
            
        else:
            times_list.append(0 + conditions["S"])
...
class Experimenter():
    ...
    def _update_I_ext_stimuli(self, condition): # stimuli_pattern = ndarray(0, 1)... etc
        ...
        if condition == "S":
            ...
            stimulus[self.anatomy_labels["brain"]: self.anatomy_labels["action1"]] = 1
        self.I_ext = stimulus
...
if __name__ == "__main__":
    ...
    project_results_dir = "experiment_results/basic_connectivity"
